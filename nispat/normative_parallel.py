    ...
    for n in range(1, number_of_batches+1):
        ...
        if testcovfile_path is None:
            if cv_folds > 1:
                ...
                raise(ValueError, """If the response file is not specified the
                                     the number of cv_folds must be larger than
                                     or equal to two(2)""")
    ...
    ** Output:
    ...
    if testcovfile_path is None:
        if cv_folds > 1:
            ...
            raise(ValueError, """If the response file is not specified cv_folds
