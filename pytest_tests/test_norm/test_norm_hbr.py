...
def test_normhbr_from_args(norm_args: dict[str, str], sample_args: dict[str, int], args: dict[str, str | bool]):
    ...
    if args.get('linear_mu', False):
        assert hbr.model.conf.mu.slope.random == args.get('random_slope_mu', False)
        assert hbr.model.conf.mu.intercept.random == args.get('random_intercept_mu', False)
        assert hbr.model.conf.mu.slope.centered == args.get('centered_slope_mu', False)
        assert hbr.model.conf.mu.intercept.centered == args.get('centered_intercept_mu', False)
...
def test_normdata_to_hbrdata(fit_data: NormData):
    ...
    assert hbrdata.batch_effects_maps == {'batch_effect_0': {0: 0, 1: 1}, 'batch_effect_1': {0: 0, 1: 1}}
