...
def np_arrays():
    ...
    batch_effects = np.concatenate([np.random.choice([0,1],(1000,1)),np.random.choice([0,2,1],(1000,1))],axis=1)
...
def test_create_data_from_dataframe(dataframe):
    norm_data = NormData.from_dataframe('test', dataframe, covariates=['X1','X2'], batch_effects=['batch1','batch2'], y='y')
    assert norm_data.X.shape == (1000,2)
    assert norm_data.y.shape == (1000,)
    assert norm_data.batch_effects.shape == (1000,2)
    assert norm_data.covariates.shape == (2,)
    ...
    assert norm_data.coords['batch_effect_dims'].to_numpy().tolist() == ['batch1','batch2'] 
    assert norm_data.attrs == {}
...
def test_split(dataframe):
    ...
    assert splits[0].coords['batch_effect_dims'].to_numpy().tolist() == ['batch1','batch2']
    ...
    assert splits[1].coords['batch_effect_dims'].to_numpy().tolist() == ['batch1','batch2']
