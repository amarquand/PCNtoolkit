...
Y_train = - 2 * X_train**2 + 2 * X_train + 1 + np.random.randn(sample_num,1)
X_test = np.random.rand(sample_num,1) * 5 - 2
Y_test = - 2 * X_test**2 + 2 * X_test + 1 + np.random.randn(sample_num,1)
...
configparam['m'] = 1000
configparam['dp'] = 0.2
configparam['hidden_neuron_num'] = 10 #15
configparam['r_dim'] = 5 # 7
configparam['z_dim'] = 3 # 5
...
ax1.errorbar(X_test, y_hat, yerr=np.sqrt(ys2).squeeze(), fmt='.',c='y',alpha=0.2, label='Uncertainty')
