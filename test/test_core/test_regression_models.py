...
class TestBLR:
    ...
    def setup(self, synthetic_data):
        ...
        self.unique_batch_effects = {k: np.unique(self.norm_data.batch_effects.sel(batch_effect_dims=k).to_numpy()) for k in self.norm_data.batch_effect_dims.to_numpy()}
        ...
        for i in range(self.norm_data.batch_effects.shape[1]):
            for j in range(self.norm_data.batch_effects.shape[0]):
                self.mapped_batch_effects[j, i] = self.be_maps[f"batch_effect_{i}"][str(self.norm_data.batch_effects[j, i].to_numpy().item())]
...
class TestHBR:
    ...
    def test_prior_creation(self):
        ...
        prior = make_prior("linear_random", linear=True, intercept=make_prior("random", random=True), slope=make_prior("random", random=True))
        assert isinstance(prior, LinearPrior)
