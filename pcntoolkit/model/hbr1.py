...
class ParamBuilder:
    ...
    def make_param(self, name, dim = (1,), **kwargs):
        ...
        if self.configs.get(f'random_{name}'):
            if self.configs.get(f'random_{name}_slope') or self.configs.get(f'random_{name}_intercept'): # Then it is a linear model
            # First make a slope and intercept, and use those to make a linear parameterization 
                if self.configs.get(f'random_{name}_slope'):
                    if self.configs.get('centered'):
                        slope_parameterization = CentralRandomFixedParameterization(name=f'random_{name}_slope',
                                                                                    pb=self, dim=[self.feature_num], **kwargs)
                    else:
                        slope_parameterization = NonCentralRandomFixedParameterization(name=f'random_{name}_slope', 
                                                                                       pb=self, dim=[self.feature_num], **kwargs)
                if self.configs.get(f'random_{name}_intercept'):
                    if self.configs.get('centered'):
                        intercept_parameterization = CentralRandomFixedParameterization(name=f'random_{name}_intercept',
                                                                                    pb=self, dim=(1,), **kwargs)
                    else:
                        intercept_parameterization = NonCentralRandomFixedParameterization(name=f'random_{name}_intercept', 
                                                                                       pb=self, dim=(1,), **kwargs)
                return LinearParameterization(name=name, dim=dim, 
                                                ...
                                                pb=self, **kwargs)
            else: # it is a fixed parametrization (this is used usually for sigma and maybe better to use wider dist as default.)
                if self.configs.get('centered'):
...
class FixedParameterization(Parameterization):
    ...
    def __init__(self, name, dim, pb:ParamBuilder, **kwargs):
        ...
        params = kwargs.get(f'{name}_params',(0.,1.)) # should be wider I think.
