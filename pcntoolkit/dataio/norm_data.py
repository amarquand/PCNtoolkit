...
class NormData(xr.Dataset):
    ...
    def split(self, splits: Tuple[float, ...], split_names: Tuple[str, ...] = None) -> Tuple['NormData', ...]:
        ...
        if len(splits) != len(split_names):
            raise ValueError(
                "The number of splits must match the number of split names.")
        ...
        X1, X2, y1, y2, batch_effects1, batch_effects2 = train_test_split(
            self.X, self.y, self.batch_effects, test_size=splits[1], stratify=self.batch_effects, random_state=42)
        split1 = NormData(name=split_names[0], data_vars={'X': (['datapoints', 'covariates'], X1.data), 'y': (['datapoints'], y1.data), 'batch_effects': (
            ['datapoints', 'batch_effect_dims'], batch_effects1.data)}, coords={'datapoints': np.arange(X1.shape[0]), 'covariates': self.covariates, 'batch_effect_dims': self.batch_effect_dims}, attrs=self.attrs)
        split2 = NormData(name=split_names[1], data_vars={'X': (['datapoints', 'covariates'], X2.data), 'y': (['datapoints'], y2.data), 'batch_effects': (
            ['datapoints', 'batch_effect_dims'], batch_effects2.data)}, coords={'datapoints': np.arange(X2.shape[0]), 'covariates': self.covariates, 'batch_effect_dims': self.batch_effect_dims}, attrs=self.attrs)
